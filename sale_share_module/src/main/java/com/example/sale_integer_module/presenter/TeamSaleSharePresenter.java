...
public class TeamSaleSharePresenter extends MVPBasePresenter<TeamSaleShareContract.IView>
        implements TeamSaleShareContract.IPresenter {
    ...
    public void initData(int user_id, String role) {
        ...
        mModel.initTeamSaleIntegerDetail(user_id, role, new OnGetInfoListener<BaseBean<List<SaleShareRecordBean>>>() {
            @Override
    ...
    @Override
    public void loadMoreData(int user_id, String role) {
        if (!isViewAttached())
            return;
        if (!NetworkUtils.isConnected()) {//没链接的话
            getView().showErrorHint("网络错误");
        ...
        mModel.loadMoreTeamSaleIntegerDetail(user_id, role, sale_share_id, new OnGetInfoListener<BaseBean<List<SaleShareRecordBean>>>() {
            @Override
            public void onComplete() {
