package com.example.redeem_integer_module.adapter;
...
public class RedeemAdapter extends RecyclerView.Adapter<RedeemAdapter.ViewHolder> {
    ...
    private List<RedeemRecordBean> mRedeemRecordBeans;

    public RedeemAdapter(List<RedeemRecordBean> redeemRecordBeans) {
        mRedeemRecordBeans = redeemRecordBeans;
    }

    @Override
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.redeem_item_record, parent, false);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        RedeemRecordBean bean = mRedeemRecordBeans.get(position);//销毁积分bean
        spaceNum(holder.mIdText, "编号:", bean.getRedeem_id() + "");//编号
   ...
   /*     if (RedeemRecordBean.INTEGRAL_TYPE_SHIFT.equals(bean.getIntegral_type())) {
    ...
    public int getItemCount() {
        return mRedeemRecordBeans == null ? 0 : mRedeemRecordBeans.size();
    }
