...
import com.example.common_view.bean.DatePickerProperty;
...
public class DatePickerView extends View {
    ...
    private DatePickerProperty mProperty = new DatePickerProperty();//属性有关
    ...
    private void init() {
        ...
        mPaint1.setColor(mProperty.getData1Color());

        //第二个画笔
        mPaint2 = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint2.setStyle(Paint.Style.FILL);
        mPaint2.setTextAlign(Paint.Align.CENTER);
        mPaint2.setColor(mProperty.getData2Color());
        mMinInertVelocity = dipToPx(750);
    }
    ...
    protected void onSizeChanged(int w, int h, int oldW, int oldH) {
        ...
        mViewHeight = h;
        mViewWidth = w;
        mProperty.setMaxTextSize(mViewHeight / 7f);//设置最大文字尺寸
        mProperty.setMinTextSize(mProperty.getMaxTextSize() / 2.2f);//最小文字尺寸
        mMoveSpeed = h / dipToPx(16);//自动滚回中间的速度
    }

    @Override
    public boolean onTouchEvent(MotionEvent event) {

        if (!mProperty.isCanScroll())//是否可滚动
            return false;
    ...
    private void inertiaScroll() {
        ...
        float offsetY = (float) (ratio * Math.sqrt((Math.abs(mLastVelocity) - mMinInertVelocity)) / dipToPx(1));
    ...
    private void move(float eventY) {
        ...
        if (mMoveLen > MARGIN_ALPHA * mProperty.getMinTextSize() / 2) {
            ...
            mMoveLen -= MARGIN_ALPHA * mProperty.getMinTextSize();//减去
        } else if (mMoveLen < -MARGIN_ALPHA * mProperty.getMinTextSize() / 2) {
            ...
            mMoveLen += MARGIN_ALPHA * mProperty.getMinTextSize();
        }
    ...
    private void drawData(Canvas canvas) {
        ...
        float size = mProperty.getTextSizeDValue() * scale
                + mProperty.getMinTextSize();//得到文字尺寸

        mPaint1.setTextSize(size);
        mPaint1.setAlpha((int) (mProperty.getTextAlphaDValue() * scale
                + mProperty.getMinTextAlpha()));
    ...
    private void drawOtherText(Canvas canvas, int position, int type) {
        ...
        float d = MARGIN_ALPHA * mProperty.getMinTextSize() * position + type * mMoveLen;//distance间距
        float scale = parabola(mViewHeight / 4.0f, d);
        float size = (mProperty.getTextSizeDValue()) * scale +
                mProperty.getMinTextSize();
        mPaint2.setTextSize(size);

        int alpha = (int) (mProperty.getTextAlphaDValue() * scale
                + mProperty.getMinTextAlpha());
    ...
    public void setCanScroll(boolean canScroll) {
        mProperty.setCanScroll(canScroll);
    }
