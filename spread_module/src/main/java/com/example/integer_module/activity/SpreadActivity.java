...
import com.example.integer_module.fragment.SpreadFragment;
...
public class SpreadActivity extends MVPBaseActivity {
    ...
    private void initFragment() {
        ...
        mFragments.add(new SpreadFragment(SpreadRecordBean.INTEGRAL_TYPE_TRANSFERS));//互转
        mFragments.add(new SpreadFragment(SpreadRecordBean.INTEGRAL_TYPE_REGISTER));//注册用户
        mFragments.add(new SpreadFragment(SpreadRecordBean.INTEGRAL_TYPE_TOP));//充值


        List<String> titles = new ArrayList<>();
        titles.add(SpreadRecordBean.INTEGRAL_TYPE_TRANSFERS);//互转
        titles.add(SpreadRecordBean.INTEGRAL_TYPE_REGISTER);//注册用户


        titles.add(SpreadRecordBean.INTEGRAL_TYPE_TOP);//充值
        titles.add("已结束");
