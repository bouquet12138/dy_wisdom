package com.example.bonus_integer_module.adapter;
...
public class BonusAdapter extends RecyclerView.Adapter<BonusAdapter.ViewHolder> {
    ...
    private List<BonusRecordBean> mBonusRecordBeans;

    public BonusAdapter(List<BonusRecordBean> bonusRecordBeans) {
        mBonusRecordBeans = bonusRecordBeans;
    }

    @Override
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.bonus_item_record, parent, false);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        BonusRecordBean bean = mBonusRecordBeans.get(position);//销毁积分bean
        spaceNum(holder.mIdText, "编号:", bean.getBonus_id() + "");//编号
   ...
   /*     if (BonusRecordBean.INTEGRAL_TYPE_SHIFT.equals(bean.getIntegral_type())) {
    ...
    public int getItemCount() {
        return mBonusRecordBeans == null ? 0 : mBonusRecordBeans.size();
    }
